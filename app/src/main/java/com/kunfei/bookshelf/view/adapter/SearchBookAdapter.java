...
public class SearchBookAdapter extends RefreshRecyclerViewAdapter {
    ...
    public void onBindIViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        String myTitle = String.format("%s (%s)", searchBooks.get(position).getName(), searchBooks.get(position).getAuthor());
        if (TextUtils.isEmpty(searchKey) && !activity.isFinishing()) {
            searchKey = ((SearchBookActivity) activity).getSearchKey();
        }
        if (!TextUtils.isEmpty(searchKey) && myTitle.contains(searchKey)) {
            int index = myTitle.indexOf(searchKey);
            int len = searchKey.length();
            Spanned temp = Html.fromHtml(
                    "<font color=#666666>" + myTitle.substring(0, index) + "</font>"
                            + "<font color=#FF0000>"
                            + myTitle.substring(index, index + len) + "</font>"
                            + "<font color=#666666>"
                            + myTitle.substring(index + len) + "</font>");
            myViewHolder.tvName.setText(temp);
        } else {
            myViewHolder.tvName.setText(myTitle);
        }
